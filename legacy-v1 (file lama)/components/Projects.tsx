...
const projects: Project[] = [
  ...
  },
  {
    id: 2,
    title: 'OryonWeb.com',
    description: 'OryonWeb.com adalah platform layanan profesional yang berfokus pada jasa pembuatan dan pengembangan website untuk individu, bisnis, UMKM, hingga perusahaan. OryonWeb menghadirkan solusi web modern yang cepat, responsif, aman, dan dirancang sesuai kebutuhan pengguna.',
...
const cardVariants = {
  ...
  visible: { 
    opacity: 1, 
    y: 0, 
    rotateX: 0, // Stand up straight
    scale: 1,
    transition: { 
      type: "spring", 
      stiffness: 60, // Slower, heavier feel
      damping: 20,
      mass: 1.2
    }
  }
        ...
        >
          {projects.map((project) => (
            ...
            >
              ...
              {/* --- CIRCUIT BORDER TRACING ANIMATION --- */}
              {/* Top */}
              <div className="absolute top-0 left-0 w-0 h-[2px] bg-cyber-primary group-hover:w-full transition-all duration-700 ease-in-out delay-0 z-30 shadow-[0_0_10px_#00f3ff]"></div>
              {/* Right */}
              <div className="absolute top-0 right-0 w-[2px] h-0 bg-cyber-primary group-hover:h-full transition-all duration-500 ease-in-out delay-500 z-30 shadow-[0_0_10px_#00f3ff]"></div>
              {/* Bottom */}
              <div className="absolute bottom-0 right-0 w-0 h-[2px] bg-cyber-primary group-hover:w-full transition-all duration-700 ease-in-out delay-300 z-30 shadow-[0_0_10px_#00f3ff]"></div>
              {/* Left */}
              <div className="absolute bottom-0 left-0 w-[2px] h-0 bg-cyber-primary group-hover:h-full transition-all duration-500 ease-in-out delay-0 z-30 shadow-[0_0_10px_#00f3ff]"></div>


              {/* Image Section with "Cyber Shutter" Reveal */}
              <div className="relative h-52 overflow-hidden z-10 bg-cyber-black">
                ...
                <img 
                  src={project.image} 
                  alt={project.title} 
                  className="w-full h-full object-cover transform scale-100 group-hover:scale-110 transition-transform duration-[1.5s] ease-out filter grayscale group-hover:grayscale-0"
                />

                {/* 2. Cyber Shutter / Digital Veil (Slides away on hover) */}
                <div className="absolute inset-0 bg-cyber-surface/60 group-hover:bg-transparent transition-colors duration-500 z-10 mix-blend-multiply"></div>
                <div className="absolute inset-0 bg-gradient-to-tr from-cyber-primary/40 to-cyber-secondary/40 translate-y-0 group-hover:-translate-y-full transition-transform duration-700 ease-in-out z-20 mix-blend-overlay"></div>
              ...
              <div className="p-6 relative z-10 bg-cyber-surface h-full flex flex-col group-hover:bg-cyber-surface-gray/50 transition-colors duration-500">
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <ArrowRight className="text-gray-600 group-hover:text-cyber-primary transform -rotate-45 group-hover:rotate-0 transition-all duration-500" size={24} />
                </div>
