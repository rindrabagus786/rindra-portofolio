...
const Navbar: React.FC<NavbarProps> = ({ isHighContrast, toggleTheme }) => {
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    ...
    if (element) {
      // Offset calculation for fixed navbar (approx 80px)
      const offset = 80;
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          <div onClick={scrollToTop} className="flex-shrink-0 cursor-pointer flex items-center gap-2 group">
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navItems.map((item) => (
                ...
                >
                  ...
                  <span className="absolute bottom-0 left-0 w-full h-0 bg-cyber-primary group-hover:h-full transition-all duration-300 -z-0 ease-out"></span>
                </a>
              ))}
            </div>
