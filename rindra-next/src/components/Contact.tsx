...
const SocialIcon3D = memo(function SocialIcon3D({
    name,
    isShadow = false,
}: {
    name: string;
    ...
    const commonClasses = `w-full h-full transition-all duration-300 ${isShadow
            ? 'opacity-50 blur-[1px] brightness-0 translate-x-1 translate-y-1'
        ...
        case 'Email':
            return (
                <svg viewBox="0 0 24 24" className={`${commonClasses} text-cyan-400`}>
                    <path
                        fill="currentColor"
                        d="M20,4H4C2.895,4,2,4.895,2,6v12c0,1.105,0.895,2,2,2h16c1.105,0,2-0.895,2-2V6C22,4.895,21.105,4,20,4z M20,6l-8,5 L4,6H20z M4,18V8l8,5l8-5v10H4z"
                    />
                </svg>
            );
        case 'LinkedIn':
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
                    {socialLinks.map((item, index) => (
                        ...
                        >
                            <div
                                className={`absolute inset-0 bg-gradient-to-br ${item.color} rounded-2xl transform translate-y-4 scale-[0.95] blur-xl opacity-20 group-hover:opacity-60 transition-all duration-500`}
                            />

                            <div
                                className={`relative h-full bg-cyber-dark/80 backdrop-blur-xl border border-gray-800 ${item.borderColor} p-8 rounded-2xl flex flex-col items-center justify-center gap-6 text-center transition-all duration-300 group-hover:shadow-[0_0_30px_rgba(0,0,0,0.5)] overflow-hidden`}
                            >
                                ...
                                <div className="space-y-2 z-10">
                                    <h3 className="text-xl font-display font-bold text-white flex items-center justify-center gap-2">
                                        ...
                                        <ArrowUpRight
                                            size={16}
                                            className="opacity-0 -translate-x-2 translate-y-2 group-hover:opacity-100 group-hover:translate-x-0 group-hover:translate-y-0 transition-all duration-300 text-gray-400"
                                        />
                                    </h3>
                ...
                >
                    <div className="flex items-center gap-3 px-6 py-3 rounded-full bg-cyber-gray/20 border border-gray-800 text-gray-400 backdrop-blur-sm hover:border-cyber-primary/50 transition-colors duration-300">
                        ...
                        <span className="font-mono text-sm">
                            Berbasis di Madiun, Jawa Timur, Indonesia
                        </span>
